...
Validated finding: On 2026-09-04, the existing Suwayomi and MangaDex artifacts fetched an actual page as `image/jpeg` with 547,622 bytes and a temporary loopback diagnostic page visibly showed the Source Plugin name, comic title, chapter, page count, and image. A previously issued Suwayomi-local chapter/page path returned `404` after restart until chapter and page metadata were fetched again. This validates live page bytes and browser display as separate boundary evidence, and establishes that Suwayomi-local numeric identifiers and page URLs are transient; it does not validate a formal Suwayomi-to-Local-Core adapter or product WebUI.
